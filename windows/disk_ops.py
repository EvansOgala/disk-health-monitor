...
def _list_disks_linux() -> list[DiskInfo]:
    if not shutil.which("lsblk"):
        return []
    code, out = _run(["lsblk", "-dn", "-o", "NAME,TYPE,MODEL,TRAN,ROTA,SIZE"])
    if code != 0:
        return []
    disks: list[DiskInfo] = []
...
def _list_disks_windows() -> list[DiskInfo]:
    # Prefer physical devices via smartctl when available.
    smart_devices = _smartctl_scan_devices()
    if smart_devices:
        return smart_devices

    # Fallback to logical volumes only when smartctl is not installed.
    if psutil is None:
        return []
    out: list[DiskInfo] = []
    seen: set[str] = set()
    for part in psutil.disk_partitions(all=False):
        ...
        try:
            ...
            size = _human_size(int(usage.total))
        except Exception:  # noqa: BLE001
            size = "Unknown"
        out.append(
            DiskInfo(
                device=device,
                model="Local Volume",
                protocol=part.fstype or "unknown",
                size=size,
                rotational="unknown",
            )
        )
...
def _smartctl_scan_devices() -> list[DiskInfo]:
    if os.name != "nt" or not shutil.which("smartctl"):
        return []
    code, out = _run(["smartctl", "--scan-open"], timeout=30)
    ...
    for line in out.splitlines():
        ...
        code_i, out_i = _run(["smartctl", "-i", device], timeout=30)
        if code_i in {0, 2} and out_i:
            m_model = re.search(r"Model Family:\s*(.+)", out_i)
            if not m_model:
                m_model = re.search(r"Device Model:\s*(.+)", out_i)
            if not m_model:
                m_model = re.search(r"Product:\s*(.+)", out_i)
            if m_model:
                model = m_model.group(1).strip()

            m_cap = re.search(r"User Capacity:\s*([^\r\n]+)", out_i)
            if m_cap:
                cap_line = m_cap.group(1).strip()
                m_bytes = re.search(r"\[([0-9,]+)\s+bytes\]", cap_line)
                if m_bytes:
                    try:
                        size = _human_size(int(m_bytes.group(1).replace(",", "")))
                    except Exception:  # noqa: BLE001
                        size = cap_line
                else:
                    size = cap_line
            ...
            if "NVMe" in out_i:
                protocol = "nvme"
            elif "SATA" in out_i or "ATA" in out_i:
                protocol = "sata"
...
def _health_smartctl(device: str) -> tuple[str, int | None, int | None, str]:
    ...
    return _parse_smart_health(out), _parse_temp(out), _parse_power_on_hours(out), out
...
def _health_windows(device: str) -> tuple[str, int | None, int | None, str]:
    # If smartctl is installed on Windows, try it directly.
    if shutil.which("smartctl"):
        return _health_smartctl(device)

    details = (
        "Windows fallback mode:\n"
        "- Install smartmontools and add smartctl to PATH for SMART health details.\n"
        "- Current mode reports availability and basic disk usage only."
    )
    return "Unknown", None, None, details
...
def read_all_disk_health(alert_temp_c: int = 60) -> list[DiskHealth]:
    ...
    for disk in list_disks():
        if os.name == "nt":
            health, temp, poh, details = _health_windows(disk.device)
        else:
